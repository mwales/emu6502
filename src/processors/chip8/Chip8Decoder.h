...
protected:
   ...
   void setError(std::string errMsg);


   /**
    * A function that the decoder can call to get more op-codes
    */
   virtual bool getByteFromAddress(CpuAddress address, uint8_t* retByte) = 0;
   ...
   */
   ...
   virtual void insStoreRegsToIndexMemoryXo(unsigned regStart, unsigned regStop) = 0;

   virtual void insLoadRegsFromIndexMemoryXo(unsigned regStart, unsigned regStop) = 0;
   ...
   virtual void insScrollUp(unsigned char numLines) = 0;
   ...
   virtual void insGraphicsPlane(unsigned char planeNum) = 0;
