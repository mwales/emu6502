...
// The next prefix is also valid for XO chip load and save instruction
...
#define OPCODE_SCROLL_UP          0x00d0
#define SCROLL_MASK               0x00f0
...
{
   uint8_t lsb, msb;
   if (!getByteFromAddress(addr, &lsb))
   {
	   setError("Error retreiving opcode bytes at " + addressToString(addr));
	   return false;
   }

   if (!getByteFromAddress(addr+1, &msb))
   {
...
std::string getErrorMessge()
{
	return "Unknown error";
}

void Chip8Decoder::setError(std::string errMsg)
{
	if (theLastError.empty())
	{
		theLastError = errMsg;
	}
}
...
bool Chip8Decoder::decodeAddressBasedIns(unsigned int opCode)
{
...
bool Chip8Decoder::decodeParamlessIns(unsigned int opCode)
{
   // Special case instruction scroll up/down
   if ( (opCode & SCROLL_MASK) == OPCODE_SCROLL_DOWN)
   {
   ...
   if ( (opCode & SCROLL_MASK) == OPCODE_SCROLL_UP)
   {
      const unsigned int SCROLL_UP_NUM_LINES_MASK = 0x000f;
      unsigned char numLines = opCode & SCROLL_UP_NUM_LINES_MASK;
      insScrollUp(numLines);
      return true;
   }
...
bool Chip8Decoder::decodeRegisterConstantIns(unsigned int opCode)
{
...
bool Chip8Decoder::decodeTwoRegistersIns(unsigned int opCode)
{
   ...
   switch(noParmOpCode)
   {
      ...
      case 0x5002:
         insStoreRegsToIndexMemoryXo(dest, src);
         return true;

      case 0x5003:
         insLoadRegsFromIndexMemoryXo(dest, src);
         return true;
...
bool Chip8Decoder::decodeOneRegisterIns(unsigned int opCode)
{
...
bool Chip8Decoder::decodeSpriteIns(unsigned int opCode)
{
